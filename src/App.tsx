...
import { Camera, Square, Play, Music, Loader2, AlertCircle, Activity, Cpu, ScanFace, Info, X } from 'lucide-react';
...
export default function App() {
        ...
        if (stateString !== pendingStateRef.current) {
          ...
          vibeTimeoutRef.current = setTimeout(() => {
            if (stateString !== lastStateRef.current) {
              ...
              const newVibe = getVibeForObjects(classesArray) + `, ${currentEmotion} mood`;
  ...
  const startSession = async () => {
    ...
    try {
      ...
      // Start detection immediately directly on local engine
      if (!isPlayingRef.current) {
        isPlayingRef.current = true;
        detectLoopRef.current = requestAnimationFrame(runDetection);
      }

      setStatus('Starting Local Music Engine...');
      
      const engine = new ProceduralMusicEngine();
      proceduralEngineRef.current = engine;
      engine.start();

      setStatus('Connected & Playing');
      setIsPlaying(true);
      
      const initialPrompt = "minimalist ambient drone, quiet, neutral mood";
      setCurrentPrompt(initialPrompt);
      lastPromptRef.current = initialPrompt;
      engine.setVibe(initialPrompt);
  ...
  const stopSession = (closeCamera: boolean = true) => {
    ...
    if (status === 'Connected & Playing' || status === 'Starting Local Music Engine...' || status.includes('Local Synth')) {
      setStatus('Idle');
    }
        ...
        <div className="flex flex-col lg:flex-row justify-between gap-4 min-h-full">
              ...
              <div className="flex flex-col items-start gap-4 shrink-0">
                <div className="flex items-start justify-between w-full">
                  <div>
                    ...
                    <p className="text-[10px] text-white/70 font-mono uppercase tracking-widest">Procedural Synth Engine</p>
                  </div>
          ...
          <div className="contents lg:flex lg:flex-col lg:justify-between lg:items-end w-full lg:w-80 pointer-events-none shrink-0 mt-0">
            ...
            <div className="flex flex-col gap-4 shrink-0 w-full order-2 lg:order-none pointer-events-auto">
              ...
              {/* Audio Profile */}
              <div className="order-2 lg:order-1 w-full bg-black/40 backdrop-blur-md border border-white/20 p-5 shadow-[0_0_30px_rgba(0,0,0,0.8)]" title="Procedural musical prompt based on your environment and mood">
            ...
            >
              ...
              <div className="space-y-4 text-sm text-white/80 leading-relaxed">
                ...
                <ul className="list-disc pl-5 space-y-2 text-white/70">
                  ...
                  <li><strong>Audio Profile:</strong> A completely local, procedural synth engine creates musical soundscapes matching the detected vibe.</li>
                </ul>
                <p className="text-xs text-white/50 mt-4 pt-4 border-t border-white/10">
                  Note: All processing happens locally in your browser. No video data or API calls are used.
                </p>
              </div>
            </motion.div>
          </motion.div>
